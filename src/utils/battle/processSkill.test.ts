...
describe("process skills", () => {
  ...
  describe("skills triggering effect", () => {
    test("attack power buff based on self attack", () => {
      const selected = G.lineups["0"][0];
      ...
      target.forEach((c, ind) => {
        expect(c.effects.length).toBe(1);
        expect(c.effects[0].value).toBe(attackBuff[ind]);
        expect(c.ATK).toBe(c.baseATK + attackBuff[ind]);
      });
    });

    test("attack power buff based on target attack", () => {
      const selected = G.lineups["0"][0];
      const target = G.lineups["0"];
      const skill = {
        condition: SkillCondition.ATTACK,
      ...
      target.forEach((c) => {
        expect(c.effects.length).toBe(1);
        expect(c.effects[0].value).toBe(skill.value);
        expect(c.ATK).toBe(Math.floor(c.baseATK * (1 + skill.value)));
      });
    });
  ...
  describe("skills triggering action", () => {
    test("should push extra skills", () => {
      ...
      expect(selected.effects.length).toBe(0);
      expect(selected.extraSkill).toEqual(expect.arrayContaining([extraSkill]));
    });
    ...
    test("should clear attack debuffs", () => {
      ...
      expect(target.effects).toEqual(expect.arrayContaining(buffs.slice(0, 1)));
    });
    ...
    test("should clear sustain debuffs", () => {
      ...
      expect(target.effects.length).toBe(1);
      expect(target.effects).toEqual(expect.arrayContaining(buffs.slice(1, 2)));
    });
    ...
    test("should clear specified effects from self", () => {
      ...
      expect(selected.effects).toEqual(expect.arrayContaining(buffs.slice(1)));
    });
    ...
    describe("skills dealing damage", () => {
      ...
      test("should trigger counterstrike", () => {
        ...
        const skill = normalAttack;
        const damage = calcDamage(selected, target, skill);
        const counterstrikeDamage = calcDamage(target, selected, counterstrike);
        const log = [] as ILog[];
        const expectedLog = [
          {
            player: "0",
...
describe("validate targets", () => {
  test("should target self", () => {
    ...
    expect(targets).toEqual(expect.arrayContaining(G.lineups["0"].slice(0, 1)));
  });
  ...
  test("should target specified indices", () => {
    ...
    expect(targets).toEqual(
      expect.arrayContaining(
        G.lineups["0"].filter((_, i) => (skill.target as number[]).includes(i))
      )
    );
  });
  ...
  describe("should target specified attributes", () => {
    ...
    attributes.forEach((attr) => {
      test(`should target ${attr.name} attribute teammates`, () => {
        const skill = { target: attr.target } as ISkill;
        const { targets, isEnemy } = getSkillTargets(G, ctx, skill);
        ...
        expect(targets).toEqual(
          expect.arrayContaining(
            G.lineups["0"].filter((c) => c.attribute === attr.attribute)
          )
        );
      });
    });
  ...
  describe("should target specified positions", () => {
    ...
    positions.forEach((p) => {
      test(`should target ${p.name}s`, () => {
        const skill = { target: p.target } as ISkill;
        const { targets, isEnemy } = getSkillTargets(G, ctx, skill);
        ...
        expect(targets).toEqual(
          expect.arrayContaining(
            G.lineups["0"].filter((c) => c.position === p.position)
          )
        );
      });
    });

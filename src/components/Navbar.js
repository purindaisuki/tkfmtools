  ...
  return (
    <div>
      ...
      {navbarContent(userLanguage, isDefault).map((item, ind) =>
        ind === 0 ? null : item.expandable ? (
          <StyledDropDown
            button={
              <TextButton disableFocusRipple>
            ...
            renderItem={(dropdownItem) =>
              item.linkType === "internal" ? (
                <StyledLink to={dropdownItem.to}>
                  {dropdownItem.description}
                </StyledLink>
              ) : (
                <StyledA href={dropdownItem.to} target="_blank" key={ind}>
                  {dropdownItem.description}
                </StyledA>
              )
            }
      ...
      {({ location }) => {
        ...
        return (
          <StyledDropDown
            ...
            items={Object.entries(langConfig).map(([id, item]) => ({
              id: id,
              path: item.default ? path : "/" + id + path,
              text: langDropdownText[id],
            }))}
            renderItem={(item) => (
              <StyledLink to={item.path} disableLocale>
                {item.text}
              </StyledLink>
            )}
            closeOnclick

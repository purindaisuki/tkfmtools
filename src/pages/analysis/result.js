...
const calcLvStats = (names, array) => {
  ...
  return {
    ...
    [names[2]]:
      len === 0
        ? 0
        : Math.round((array.reduce((a, b) => a + b, 0) * 10) / len) / 10,
    [names[3]]:
      len === 0 ? 0 : len % 2 ? array[mid] : (array[mid - 1] + array[mid]) / 2,
...
const calcPR = (chars) => {
  ...
  const PRArray = chars.map((c) => {
    ...
    return {
      ...
      PR:
        Math.floor(
          (1 -
            (array.length - findIndex(array, c.cp) + 0.5) /
              (array.length + 1)) *
            1000
        ) / 10,
      cp: c.cp,
  ...
  PRArray.sort((a, b) =>
    a.PR - b.PR !== 0 ? a.PR - b.PR : a.cp / 1000000 - b.cp / 1000000
  );
  // only need top 5 and bottom 5
  PRArray.splice(5, PRArray.length - 10);
  PRArray.forEach((c) => {
    c.cp = formatNumber(c.cp);
  });
  // total PR
  ...
  PRArray.push({
    ...
    PR:
      Math.floor(
        (1 -
          (array.length - findIndex(array, totalCp) + 0.5) /
            (array.length + 1)) *
          1000
      ) / 10,
  });
...
const calcLineupStats = (state, string, chart) => {
  ...
  string.tags.slice(0, 5).forEach((t) =>
    treeMapDataByAttribute.children.push({
      name: t,
      children: [],
    })
  );
  ...
  return {
    radarDataByPosition: radarDataByPosition.map((group, ind) =>
      calcLvStats([string.tags[ind + 5], ...chart[2].legend], group)
    ),
    radarDataByAttribute: radarDataByAttribute.map((group, ind) =>
      calcLvStats([string.tags[ind], ...chart[3].legend], group)
    ),
...
const Analysis = () => {
  ...
  const isSm = typeof window !== "undefined" ? window.innerWidth < 600 : false;
    ...
    <>
      ...
      <ChartsContainer ref={componentRef}>
        ...
        <ChartWrapper>
          ...
          <RankContainer
            rank="high"
            chars={lineupStats.PRArray.slice(-6, -1).reverse()}
          />
        </ChartWrapper>
        <ChartWrapper>
          ...
          <RankContainer
            rank="low"
            chars={lineupStats.PRArray.slice(0, -1).slice(0, 5)}
          />
        </ChartWrapper>
      </ChartsContainer>
      <TextModal
